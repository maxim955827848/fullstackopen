...
Below is the sequence diagram depicting the process when a user navigates to the SPA version of the notes app.
...
sequenceDiagram
    ...
    activate server
    server-->>browser: data.json array containing the notes
    deactivate server

    Note over browser: The browser executes the callback function and renders the notes using DOM manipulation